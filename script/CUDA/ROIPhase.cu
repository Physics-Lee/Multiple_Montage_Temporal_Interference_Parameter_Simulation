...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    for (int i = 0; i<Nloop; i++){
        ...
        switch(alpha){
            case -1:
                ROIMaxKernel<<<gridSize, blockSize,sharedMemSize>>>(Ki, Cbase, N128, d_e0, Nelec, d_elec, Ncu, d_r2);
                break;
            case 0:
                ROIVolumeKernel<<<gridSize, blockSize,sharedMemSize>>>(Ki, Cbase, N128, d_e0, Nelec, d_elec, Ncu, d_volume, thres, d_r2);
                break;
            default:
                ROIAlphaKernel<<<gridSize, blockSize,sharedMemSize>>>(Ki, Cbase, N128, d_e0, Nelec, d_elec, Ncu, d_volume, alpha, d_r2);
        }
    ...
    if(vflag){
        ...
        mexEvalString("drawnow") ;
        mexPrintf("ROI phase GPU part end...\n");
    }
}
